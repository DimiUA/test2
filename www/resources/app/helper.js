...
Protocol = {
    ...
    Helper: {
        ...
        getAddressByGeocoder: function(latlng,replyFunc){
            ...
            //jsonRequest(url, function(result){ replyFunc(result.display_name);});
            var coords = LANGUAGE.COM_MSG09 + ': ' + latlng.lat + ', ' + LANGUAGE.COM_MSG10 + ': ' + latlng.lng;
            $.ajax({
                   type: "GET",
                    url: "http://map.quiktrak.co/reverse.php?format=json&lat={0}&lon={1}&zoom=18&addressdetails=1".format(latlng.lat, latlng.lng),                
               dataType: "json",
                  async: true, 
                  cache: false,
                ...
                error: function(XMLHttpRequest, textStatus, errorThrown){ 
                    $.ajax({
                           type: "GET",
                            url: "http://nominatim.openstreetmap.org/reverse?format=json&lat={0}&lon={1}&zoom=18&addressdetails=1".format(latlng.lat, latlng.lng),                
                       dataType: "json",
                          async: true, 
                          cache: false,
                        success: function (result) { 
        ...
        getLatLngByGeocoder: function(address,replyFunc){
                var url = "http://nominatim.openstreetmap.org/search?q={0}&format=json&polygon=1&addressdetails=1".format(address);
        ...
        createMap: function(option){
            var osm = L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', { name: 'osm', attribution: '' });
            ...
            */
            ...
            if(asset.posInfo.positionTime === null) {
                ...
                if(asset.haveFeature("Mileage")) {                    
                    ret.milage = {};
                    ret.milage.value = (Protocol.Helper.getMileageValue(asset.Unit, asset.posInfo.mileage) + parseInt(asset.InitMilage)) + '&nbsp;' + Protocol.Helper.getMileageUnit(asset.Unit);  
                }
                ...
                if(asset.haveFeature("Battery")){
                    ...
                    if (asset.posInfo.Battery) {
                        ret.battery.value = asset.posInfo.Battery + '&nbsp;%';
                    }else{
